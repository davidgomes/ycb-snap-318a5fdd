...
blobs:
  ...
  - # Cloud provider name:
    ...
    # files, when the error reports Timeout() or Temporary() as true.
    #
    # max_delay caps every wait. Every try sends the full artifact again.
    # Bucket-open tries are not recorded as publish attempts.
    #
