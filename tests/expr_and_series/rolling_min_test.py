...
def test_rolling_min_expr(
    ...
    result = df.select(nw.col("a").rolling_min(**kwargs).alias(name))

    assert_equal_data(result, {name: expected})


@pytest.mark.parametrize("kwargs_and_expected", kwargs_and_expected)
def test_rolling_min_series(
    constructor_eager: ConstructorEager, kwargs_and_expected: dict[str, Any]
) -> None:
    name = kwargs_and_expected["name"]
    kwargs = kwargs_and_expected["kwargs"]
    ...
    result = df.select(df["a"].rolling_min(**kwargs).alias(name))

    assert_equal_data(result, {name: expected})


def test_rolling_min_window_larger_than_series(
    constructor_eager: ConstructorEager,
) -> None:
    df = nw.from_native(constructor_eager({"a": [3, 1, 2]}), eager_only=True)
    result = df.select(
        x=nw.col("a").rolling_min(5, min_samples=1),
        y=nw.col("a").rolling_min(5, min_samples=1, center=True),
    )
    assert_equal_data(result, {"x": [3, 1, 1], "y": [1, 1, 1]})
