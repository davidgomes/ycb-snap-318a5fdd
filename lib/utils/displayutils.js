...
function bailOutLine(bailSummary) {
  let count = bailSummary.failures;
  return `Bail out! ${bailSummary.reason} (${count} ${count === 1 ? 'failure' : 'failures'})`;
}

exports.bailOutLine = bailOutLine;

function summaryDisplay() {
  let bailSummary = this.bailSummary;
  let lines = [];

  if (bailSummary) {
    lines.push(bailOutLine(bailSummary));
  }
