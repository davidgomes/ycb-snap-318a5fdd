...
function bailOutLine(bailInfo) {
  let count = bailInfo.failureCount;
  let reason = String(bailInfo.reason).replace(/[\r\n]+/g, ' ');

  return `Bail out! ${reason} (${count} test failure${count === 1 ? '' : 's'})`;
}

exports.bailOutLine = bailOutLine;

function bailSummaryLines(bailInfo) {
  return [
    '# bailed',
    '# ran before bail ' + bailInfo.testsRanBeforeBail,
    '# suppressed ' + bailInfo.suppressedAfterBail
  ];
}

exports.bailSummaryLines = bailSummaryLines;
...
function summaryDisplay() {
  ...
  if (this.bailInfo) {
    lines = lines.concat(bailSummaryLines(this.bailInfo));
