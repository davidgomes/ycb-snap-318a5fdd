...
function summaryDisplay() {
  ...
  if (this.bailed) {
    lines.push(bailOutLine(this.bailReason, this.bailedTests));
    lines.push('# bailed');
    lines.push('# ran before bail ' + this.testsRanBeforeBail);
    lines.push('# suppressed ' + this.suppressedAfterBail);
  }
  return lines.join('\n');
}

function bailOutLine(reason, count) {
  let text = (reason === null || reason === undefined) ? '' : String(reason);
  let failureCount = (count === null || count === undefined) ? 0 : count;
  return 'Bail out! ' + text + ' # ' + failureCount;
}
