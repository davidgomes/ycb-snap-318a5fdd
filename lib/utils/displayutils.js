...
function summaryDisplay() {
  ...
  if (this.showLauncherSummary && this.launcherStats) {
    lines.push('# Per-launcher summary');
    Object.keys(this.launcherStats).forEach(name => {
      let stats = this.launcherStats[name];
      lines.push('# ' + name + ': ' + stats.tests + ' tests, ' + stats.pass + ' pass, ' + stats.fail + ' fail, ' + stats.skip + ' skip');
    });
  }
