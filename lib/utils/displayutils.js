...
function summaryDisplay() {
  ...
  if (this.bailState) {
    lines.push('# bailed');
    lines.push('# ran before bail ' + this.bailState.testsRanBeforeBail);
    lines.push('# suppressed ' + this.bailState.suppressedAfterBail);
  }
...
function bailOutLine(bailState) {
  let count = bailState.failureCount;
  return `Bail out! ${bailState.reason} (${count} ${count === 1 ? 'failure' : 'failures'})`;
}
