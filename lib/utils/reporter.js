...
class Reporter extends EventEmitter {
  ...
  report(name, result) {
    ...
    let isFailure = !result.passed && !result.skipped && !result.todo;
    if (isFailure) {
      let testName = (result && result.name) || 'unknown test';
      let launcher = name || 'unknown';
      this.failureCount++;
      this.failedTests.push(testName);
      this.failuresByLauncher[launcher] = (this.failuresByLauncher[launcher] || 0) + 1;

      if (this.bailThreshold > 0 && this.failureCount >= this.bailThreshold) {
        this.bailed = true;
        this.bailReason = testName;
        this.bailLauncher = launcher;
        this.testsRanBeforeBail = this.total;
        this.emit('test-failure', launcher, result);
      }
    }
  }

  finish() {
    let bailInfo = this.bailed ? {
...
module.exports.parseBailThreshold = parseBailThreshold;
