...
module.exports = class BrowserTestRunner {
  ...
  abort() {
    ...
    this.aborted = true;
    log.info(`Aborting browser ${this.name()}.`);

    if (this.socket) {
      this.socket.emit('abort-tests');
    }

    this.abortPromise = Bluebird.resolve(this.finish()).catch(err => {
      log.info(`Ignoring error while aborting browser ${this.name()}: ${err}`);
    });
    return this.abortPromise;
  }
