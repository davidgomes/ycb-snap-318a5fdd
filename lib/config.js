...
class Config {
  ...
  validateReportFile() {
    ...
    let known = { '<launcher>': true, '<date>': true, '<timestamp>': true };
    matches.forEach(token => {
      if (!known[token]) {
        errors.push('Unknown template variable ' + token + ' in report_file');
      }
    });

    if (String(reportFile).indexOf('<launcher>') !== -1 && !path.extname(reportFile)) {
