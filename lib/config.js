...
class Config {
  ...
  hasAnyReportTemplate() {
    const ReportFile = require('./utils/report-file');
    return ReportFile.hasAnyTemplate(this.get('report_file'));
  }
  ...
  validateReportFile() {
    ...
    ReportFile.findUnknownTemplates(reportFile).forEach(template => {
      errors.push('Unknown template variable ' + template + ' in report_file');
    });

    if (ReportFile.hasLauncherTemplate(reportFile)) {
      const samplePath = ReportFile.expandPath(reportFile, { launcher: 'sample' });
      if (!path.extname(samplePath)) {
