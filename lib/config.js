...
class Config {
  ...
  hasLauncherTemplate() { return ReportFile.hasLauncherTemplate(this.get('report_file')); }
  hasDateTemplate() { return ReportFile.hasDateTemplate(this.get('report_file')); }
  hasTimestampTemplate() { return ReportFile.hasTimestampTemplate(this.get('report_file')); }
  hasAnyReportTemplate() { return this.hasLauncherTemplate() || this.hasDateTemplate() || this.hasTimestampTemplate(); }
  validateReportFile() {
    const reportFile = this.get('report_file');
    const result = { valid: true, errors: [], warnings: [] };
    if (!reportFile) return result;
    const tokens = reportFile.match(/<[^>]+>/g) || [];
    tokens.filter(token => ['<launcher>', '<date>', '<timestamp>'].indexOf(token) === -1)
      .forEach(token => result.errors.push(`Unknown report_file template: ${token}`));
  ...
  getExpandedReportFile(launcher) {
    const reportFile = this.get('report_file');
    return reportFile ? ReportFile.expandPath(reportFile, { launcher: launcher && launcher.name }) : null;
  }
