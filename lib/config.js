...
class Config {
  ...
  validateReportFile() {
    ...
    const warnings = [];
    if (reportFile) {
      const templates = reportFile.match(/<[^>]+>/g) || [];
      templates.forEach(template => {
        if (['<launcher>', '<date>', '<timestamp>'].indexOf(template) === -1) {
          errors.push('Unknown report_file template: ' + template);
        }
      });
      if (this.hasLauncherTemplate() && !path.extname(reportFile.replace(/<launcher>/g, ''))) {
        warnings.push('report_file templates with <launcher> should include a file extension');
      }
    }
  ...
  getExpandedReportFile(launcher) {
    return ReportFile.expandPath(this.get('report_file'), {
      launcher: launcher && (launcher.name || launcher)
    });
  }
