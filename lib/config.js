...
class Config {
  ...
  validateReportFile() {
    ...
    let match;
    while ((match = templatePattern.exec(reportFile)) !== null) {
      if (ReportFile.TEMPLATE_VARIABLES.indexOf(match[1]) === -1) {
        result.errors.push(`Unknown template variable "${match[0]}" in report_file "${reportFile}". Supported: ${ReportFile.TEMPLATE_VARIABLES.map(v => `<${v}>`).join(', ')}.`);
      }
    }

    if (this.hasLauncherTemplate()) {
      let sample = ReportFile.expandPath(reportFile, { launcher: 'launcher' });
      if (!path.extname(sample)) {
        result.warnings.push(`report_file "${reportFile}" uses <launcher> but has no file extension.`);
      }
    }
  ...
  getExpandedReportFile(launcher) {
    ...
    return ReportFile.expandPath(reportFile, { launcher });
  }
