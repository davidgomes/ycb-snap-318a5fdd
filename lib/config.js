...
class Config {
  ...
  hasLauncherTemplate() {
    return ReportFile.hasLauncherTemplate(this.reportFilePath());
  }

  hasDateTemplate() {
    return ReportFile.hasDateTemplate(this.reportFilePath());
  }

  hasTimestampTemplate() {
    return ReportFile.hasTimestampTemplate(this.reportFilePath());
  }
  ...
  validateReportFile() {
    let reportFile = this.reportFilePath();
    let errors = [];
    let warnings = [];
    ...
    let seen = {};
    let pattern = /<([^<>]+)>/g;
    let match = pattern.exec(reportFile);
    while (match) {
      let token = match[0];
      let name = match[1];
      if (!ReportFile.knownTemplates()[name] && !seen[token]) {
        seen[token] = true;
        errors.push('Unknown report_file template variable: ' + token);
      }
      match = pattern.exec(reportFile);
    }

    if (ReportFile.hasLauncherTemplate(reportFile) && !path.extname(reportFile)) {
      warnings.push('report_file <launcher> template should include a file extension');
    }
  ...
  getExpandedReportFile(launcher) {
    ...
    return ReportFile.expandPath(reportFile, options);
  }
}
