...
class Config {
  ...
  validateReportFile() {
    ...
    let templates = String(reportFile).match(/<[^>]+>/g) || [];
    templates.forEach(token => {
      let name = token.slice(1, -1);
      if (!ReportFile.knownTemplates()[name]) {
        errors.push('Unknown report_file template: ' + token);
      }
    });

    if (ReportFile.hasLauncherTemplate(reportFile) && !path.extname(reportFile)) {
      warnings.push('report_file template <launcher> is missing a file extension');
    }
