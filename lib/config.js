  ...
  validateReportFile() {
    ...
    while ((match = templatePattern.exec(reportFile)) !== null) {
      if (ReportFile.TEMPLATE_VARIABLES.indexOf(match[1]) === -1) {
        result.errors.push(`Unknown template variable ${match[0]} in report_file "${reportFile}". Supported variables: ${ReportFile.TEMPLATE_VARIABLES.map(v => `<${v}>`).join(', ')}.`);
      }
    }

    if (ReportFile.hasLauncherTemplate(reportFile) && !/\.[^./\\<>]+$/.test(path.basename(reportFile))) {
