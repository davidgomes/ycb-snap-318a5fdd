...
class Config {
  ...
  validateReportFile() {
    ...
    if (reportFile) {
      ...
      templates.forEach(t => {
        if (ReportFile.KNOWN_TEMPLATES.indexOf(t) === -1) {
          errors.push(`Unknown template variable ${t} in report_file`);
        }
      });
      if (ReportFile.hasLauncherTemplate(reportFile) && !path.extname(String(reportFile))) {
