...
class Config {
  ...
  validateReportFile() {
    ...
    if (reportFile && typeof reportFile !== 'string') {
      errors.push(`report_file must be a string, got ${typeof reportFile}.`);
    } else if (reportFile) {
      let templatePattern = /<([^<>]*)>/g;
      let match;
      while ((match = templatePattern.exec(reportFile))) {
        if (REPORT_FILE_TEMPLATES.indexOf(match[1]) === -1) {
          errors.push(
            `Unknown template variable "${match[0]}" in report_file "${reportFile}". ` +
            `Supported variables: ${REPORT_FILE_TEMPLATES.map(name => `<${name}>`).join(', ')}.`
          );
        }
      }

      let extension = path.extname(reportFile);
      if (ReportFile.hasLauncherTemplate(reportFile) && (extension.length < 2 || extension.indexOf('<') !== -1)) {
        warnings.push(
          `report_file "${reportFile}" uses <launcher> without a file extension; ` +
          'dots in launcher names (e.g. browser versions) may be mistaken for one.'
        );
      }
    }
