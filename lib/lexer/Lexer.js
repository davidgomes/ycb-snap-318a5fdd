...
export class Lexer {
    ...
    expandShorthand(propertyName, value) {
        const result = expandShorthand(this, propertyName, value);
        if (!result) return null;
        const property = String(propertyName).toLowerCase();
        const names = Object.keys(result);
        if (!names.every(name => this.getProperty(name) || name === 'top' || name === 'right' || name === 'bottom' || name === 'left')) {
            return null;
        }
        return result;
    }
    compressShorthand(propertyName, longhands) {
        return compressShorthand(propertyName, longhands);
    }
