...
module.exports = class App extends EventEmitter {
  ...
  getExitCode() {
    ...
    if (typeof this.reporter.hasBailed === 'function' && this.reporter.hasBailed()) {
      ...
      let reason = this.reporter.bailReason;
      let e = new Error('Bail out! ' + reason + ' (' + testsRanBeforeBail + ' tests ran before bail)');
      e.hideFromReporter = true;
      return e;
    }
  ...
  abortRunners() {
    ...
    let abortPromises = (this.runners || []).map(runner => {
    ...
    this._abortRunnersPromise = Bluebird.all(abortPromises);
    return this._abortRunnersPromise;
  }
