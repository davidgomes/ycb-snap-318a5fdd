...
module.exports = class App extends EventEmitter {
  constructor(config, finalizer) {
    ...
    this.aborted = false;
    this._abortRunnersPromise = null;
  ...
  getExitCode() {
    ...
    if (typeof this.reporter.hasBailed === 'function' && this.reporter.hasBailed()) {
      let report = typeof this.reporter.getBailReport === 'function'
        ? this.reporter.getBailReport()
        : {};
      report = report || {};
      let e = new Error('Bailed out: ' + this.reporter.bailReason + ' (' + report.testsRanBeforeBail + ' tests ran before bail)');
      e.hideFromReporter = true;
      return e;
    }
  ...
  abortRunners() {
    if (this._abortRunnersPromise) {
      return this._abortRunnersPromise;
    }
    if (this.aborted) {
      return Bluebird.resolve();
    }
    ...
    this._abortRunnersPromise = Bluebird.map(this.runners, runner => {
    ...
    if (this.runners) {
      this.runners.forEach(runner => {
        if (runner) {
          ...
          runner._abortPromise = null;
          runner._exitPromise = null;
        }
      });
    }
