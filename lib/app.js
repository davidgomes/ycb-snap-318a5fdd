...
module.exports = class App extends EventEmitter {
  ...
  start(cb) {
    ...
    return Bluebird.using(SignalListeners.with(), signalListeners => {
      ...
      return Bluebird.using(Reporter.with(this, this.stdoutStream, this.reportFileName), reporter => {
        ...
        reporter.on('test-failure', () => this.onBail());
  ...
  runTests() {
    ...
    if (this.aborted || (typeof this.reporter.hasBailed === 'function' && this.reporter.hasBailed())) {
      this.resetBailState();
    }
  ...
  singleRun(timeout) {
    ...
    return Bluebird.map(this.runners, (runner) => {
      ...
      if (this.restarting || this.aborted) {
        return Bluebird.resolve();
      }
  ...
  getExitCode() {
    ...
    if (typeof this.reporter.hasBailed === 'function' && this.reporter.hasBailed()) {
      ...
      let e = new Error(`Bailed out on test failure: ${this.reporter.bailReason} (${testsRanBeforeBail} test(s) ran before bail).`);
      e.hideFromReporter = true;
      e.bailed = true;
      return e;
    }
  ...
  onBail() {
    let report = this.reporter.getBailReport();
    log.info(`Bailing out after test failure in ${report.bailLauncher}: ${this.reporter.bailReason}`);

    this.abortRunners().catch(err => log.error('Failed to abort runners.', err));
  }
  ...
  abortRunners() {
    ...
    this.abortPromise = Bluebird.all(this.runners.map(runner => Bluebird.try(() => {
