...
module.exports = class App extends EventEmitter {
  ...
  runTests() {
    ...
    if (this._runAborted || (this.reporter && typeof this.reporter.hasBailed === 'function' && this.reporter.hasBailed())) {
      this.resetBailState();
    }
  ...
  getExitCode() {
    ...
    if (typeof this.reporter.hasBailed === 'function' && this.reporter.hasBailed()) {
      ...
      let err = new Error('Bail out! ' + bailReason + ' (' + testsRanBeforeBail + ' tests ran before bail)');
      err.hideFromReporter = true;
      return err;
    }
  ...
  abortRunners() {
    ...
    return Promise.resolve(Bluebird.each(this.runners, runner => {
