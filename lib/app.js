...
module.exports = class App extends EventEmitter {
  ...
  singleRun(timeout) {
    ...
    return Bluebird.map(this.runners, (runner) => {
      ...
      if (this.restarting || this.abortPromise) {
        return Bluebird.resolve();
      }
  ...
  getExitCode() {
    ...
    if (this.reporterHasBailed()) {
      ...
      let e = new Error(`Bailed out on test failure: ${this.reporter.bailReason} (${testsRanBeforeBail} tests ran before bail).`);
      e.bailed = true;
      e.hideFromReporter = true;
      return e;
    }
  ...
  abortRunners() {
    ...
    log.info('Aborting all test runners.');

    if (this.server && typeof this.server.broadcastAbort === 'function') {
      this.server.broadcastAbort();
    }

    this.abortPromise = Bluebird.all(this.runners.map(runner => {
      if (typeof runner.abort === 'function') {
        return runner.abort();
      }
      return runner.exit();
    })).catch(err => {
      log.info(`Ignoring error while aborting runners: ${err}`);
    });
    return this.abortPromise;
  }

  reporterHasBailed() {
    return !!this.reporter && typeof this.reporter.hasBailed === 'function' && this.reporter.hasBailed();
  }
