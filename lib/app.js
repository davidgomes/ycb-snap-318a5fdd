...
module.exports = class App extends EventEmitter {
  constructor(config, finalizer) {
    ...
    if (this.reportFileName && typeof this.config.validateReportFile === 'function') {
      let validation = this.config.validateReportFile();
      validation.errors.forEach(message => log.error(message));
