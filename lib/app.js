...
module.exports = class App extends EventEmitter {
  ...
  abortRunners() {
    ...
    log.info('Bailing out: aborting all test runners.');
  ...
  getExitCode() {
    ...
    if (typeof this.reporter.hasBailed === 'function' && this.reporter.hasBailed()) {
      let ran = this.reporter.getBailReport().testsRanBeforeBail;
      let e = new Error(`Bailed out on test failure: ${this.reporter.bailReason} (${ran} tests ran before bail).`);
      e.bailed = true;
      e.hideFromReporter = true;
      return e;
    }
