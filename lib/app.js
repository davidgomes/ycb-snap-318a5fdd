...
module.exports = class App extends EventEmitter {
  ...
  singleRun(timeout) {
    ...
    return Bluebird.map(this.runners, (runner) => {
      ...
      if (this.restarting || this.runnersAborted) {
        return Bluebird.resolve();
      }
  ...
  getExitCode() {
    ...
    if (typeof this.reporter.hasBailed === 'function' && this.reporter.hasBailed()) {
      let testsRan = this.reporter.getBailReport().testsRanBeforeBail;
      let e = new Error(`Bail out! ${this.reporter.bailReason} (${testsRan} test${testsRan === 1 ? '' : 's'} ran before bail)`);
      e.hideFromReporter = true;
      return e;
    }
