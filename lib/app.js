...
module.exports = class App extends EventEmitter {
  ...
  start(cb) {
    ...
    let reportFileName = reportFileValidation.valid ? this.reportFileName : null;

    return Bluebird.using(SignalListeners.with(), signalListeners => {
      signalListeners.on('signal', err => this.exit(err));

      return Bluebird.using(Reporter.with(this, this.stdoutStream, reportFileName), reporter => {
        this.reporter = reporter;

        if (!reportFileValidation.valid) {
          throw new Error('Invalid report_file: ' + reportFileValidation.errors.join(' '));
        }
