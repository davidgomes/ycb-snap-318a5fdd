...
module.exports = class TapReporter {
  ...
  updateLauncherStats(prefix, data) {
    let launcher = prefix || 'unknown';
    let stats = this.launcherStats.get(launcher);
    if (!stats) {
      stats = { total: 0, pass: 0, skip: 0, todo: 0 };
      this.launcherStats.set(launcher, stats);
    }
  ...
  launcherSummaryDisplay() {
    ...
    this.launcherStats.forEach((stats, launcher) => {
      let fail = stats.total - stats.pass - stats.skip - stats.todo;
      lines.push(`# ${launcher}: ${stats.total} tests, ${stats.pass} pass, ${fail} fail, ${stats.skip} skip`);
    });
