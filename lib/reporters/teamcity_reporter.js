...
module.exports = class TeamcityReporter {
  ...
  finish() {
    ...
    this.out.write('\n\n');
    if (this.bailInfo) {
      const info = this.bailInfo;
      const text = `Bail out! ${info.reason}`;
      this.out.write(teamcityLine('message', {text: escape(text), status: 'ERROR'}));
      this.out.write(teamcityLine('buildStatisticValue', {key: 'bailedTests', value: 1}));
      this.out.write(teamcityLine('buildStatisticValue', {key: 'testsBeforeBail', value: info.testsRanBeforeBail}));
      this.out.write(teamcityLine('buildStatisticValue', {key: 'suppressedAfterBail', value: info.suppressedAfterBail}));
      this.out.write(teamcityLine('buildProblem', {description: escape(`${text} (${info.testsRanBeforeBail} tests ran before bail)`)}));
    }
