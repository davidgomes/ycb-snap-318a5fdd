...
module.exports = class TeamcityReporter {
  ...
  _writeBail() {
    ...
    let reason = this.bailInfo.reason;
    let bailText = 'Bail out! ' + reason + ' ' + this.bailInfo.count;

    this.out.write(teamcityLine('message', {
      text: escape(bailText),
      status: 'ERROR'
    }));
    ...
    this.out.write(teamcityLine('buildProblem', {
      description: escape('Bail out! ' + reason),
      identity: 'bail_on_test_failure'
    }));
  }
