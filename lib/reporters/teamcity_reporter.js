...
module.exports = class TeamcityReporter {
  ...
  finish() {
    ...
    this.out.write('\n\n');
    if (this.bailSummary) {
      this._displayBail(this.bailSummary);
    }
  ...
  _displayBail(bailSummary) {
    const reason = escape(bailSummary.reason);

    this.out.write(teamcityLine('message', {
      text: `Bail out! ${reason}`,
      status: 'ERROR',
    }));
    this.out.write(teamcityLine('buildStatisticValue', {
      key: 'bailedTests',
      value: bailSummary.failures,
    }));
    this.out.write(teamcityLine('buildStatisticValue', {
      key: 'testsBeforeBail',
      value: bailSummary.testsRanBeforeBail,
    }));
    this.out.write(teamcityLine('buildStatisticValue', {
      key: 'suppressedAfterBail',
      value: bailSummary.suppressedAfterBail,
    }));
    this.out.write(teamcityLine('buildProblem', {
      description: `Bailed out on test failure: ${reason}`,
    }));
  }
