...
module.exports = class TeamcityReporter {
  ...
  finish() {
    ...
    this.out.write('\n\n');
    if (this.bailInfo) {
      this._displayBail(this.bailInfo);
    }
  ...
  _displayBail(bailInfo) {
    const message = escape(displayutils.bailOutLine(bailInfo));

    this.out.write(teamcityLine('message', {text: message, status: 'ERROR'}));
    this.out.write(teamcityLine('buildStatisticValue', {key: 'bailedTests', value: bailInfo.failureCount}));
    this.out.write(teamcityLine('buildStatisticValue', {key: 'testsBeforeBail', value: bailInfo.testsRanBeforeBail}));
    this.out.write(teamcityLine('buildStatisticValue', {key: 'suppressedAfterBail', value: bailInfo.suppressedAfterBail}));
    this.out.write(teamcityLine('buildProblem', {description: message}));
  }
