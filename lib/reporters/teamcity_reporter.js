...
module.exports = class TeamcityReporter {
  ...
  finish() {
    ...
    this.out.write('\n\n');
    if (this.bailState) {
      this._displayBail(this.bailState);
    }
  ...
  _displayBail(bailState) {
    this.out.write(teamcityLine('message', {
      text: escape(`Bail out! ${bailState.reason}`),
      status: 'ERROR',
    }));
    this.out.write(teamcityLine('buildStatisticValue', {
      key: 'bailedTests',
      value: bailState.failureCount,
    }));
    this.out.write(teamcityLine('buildStatisticValue', {
      key: 'testsBeforeBail',
      value: bailState.testsRanBeforeBail,
    }));
    this.out.write(teamcityLine('buildStatisticValue', {
      key: 'suppressedAfterBail',
      value: bailState.suppressedAfterBail,
    }));
    this.out.write(teamcityLine('buildProblem', {
      description: escape(`Bailed out after ${bailState.failureCount} test failure(s): ${bailState.reason}`),
    }));
  }
