...
var bailOutLine = require('../utils/displayutils').bailOutLine;
...
module.exports = class XUnitReporter {
  ...
  summaryDisplay() {
    ...
    if (this.bailed) {
      rootNode.setAttribute('errors', `${this.bailedTests || 1}`);
    }
    ...
    if (this.bailed) {
      ...
      var bailError = doc.createElement('error');
      bailError.setAttribute('message', bailOutLine(this.bailReason, this.bailedTests));
      rootNode.appendChild(bailError);

      var systemOut = doc.createElement('system-out');
      var summary = [
        bailOutLine(this.bailReason, this.bailedTests),
        '# bailed',
        '# ran before bail ' + this.testsRanBeforeBail,
        '# suppressed ' + this.suppressedAfterBail
      ].join('\n');
      systemOut.appendChild(doc.createTextNode(summary));
