...
module.exports = class XUnitReporter {
  ...
  summaryDisplay() {
    ...
    if (this.bailState) {
      ...
      rootNode.appendChild(this.getBailPropertiesNode(doc, this.bailState));
    }
    ...
    if (this.bailState) {
      ...
      errorNode.setAttribute('type', 'bail');
      errorNode.setAttribute('message', `Bail out! ${this.bailState.reason}`);
      rootNode.appendChild(errorNode);

      var systemOutNode = doc.createElement('system-out');
      systemOutNode.appendChild(doc.createTextNode(this.bailSummary(this.bailState)));
      rootNode.appendChild(systemOutNode);
    }
    return doc.documentElement.toString();
  }

  getBailPropertiesNode(document, bailState) {
    var propertiesNode = document.createElement('properties');
    var properties = {
      bailReason: bailState.reason,
      testsBeforeBail: bailState.testsRanBeforeBail,
      suppressedAfterBail: bailState.suppressedAfterBail
    };

    Object.keys(properties).forEach(name => {
      var propertyNode = document.createElement('property');
      propertyNode.setAttribute('name', name);
      propertyNode.setAttribute('value', `${properties[name]}`);
  ...
  bailSummary(bailState) {
    return [
      `Bailed out after ${bailState.failureCount} test failure(s): ${bailState.reason}`,
      `Tests ran before bail: ${bailState.testsRanBeforeBail}`,
      `Suppressed after bail: ${bailState.suppressedAfterBail}`
    ].join('\n');
  }
