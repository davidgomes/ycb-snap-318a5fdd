...
module.exports = class XUnitReporter {
  ...
  getLauncherStats() {
    return this.launcherStats;
  }

  recordLauncherResult(launcher, data) {
    let key = launcher || 'unknown';
    let stats = this.launcherStats[key];
    if (!stats) {
      stats = this.launcherStats[key] = { total: 0, pass: 0, fail: 0 };
    }
  ...
  getPropertiesNode(document) {
    var propertiesNode = document.createElement('properties');
    var launchers = Object.keys(this.launcherStats);
    ...
    var launcher = this.launcherName || (launchers.length === 1 ? launchers[0] : null);
    if (launcher) {
      addProperty('launcher', launcher);
    }
    addProperty('launchers', launchers.join(','));

    launchers.forEach(name => {
      addProperty(`${name}_pass`, this.launcherStats[name].pass);
      addProperty(`${name}_fail`, this.launcherStats[name].fail);
    });
  ...
  report(prefix, data) {
    ...
    this.display();
    this.recordLauncherResult(prefix, data);
    this.total++;
  ...
  summaryDisplay() {
    ...
    if (this.includeLauncherProperties) {
      rootNode.appendChild(this.getPropertiesNode(doc));
    }
