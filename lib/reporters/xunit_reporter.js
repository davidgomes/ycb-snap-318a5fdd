...
module.exports = class XUnitReporter {
  ...
  getLauncherStats() {
    ...
    this.results.forEach(entry => {
      ...
      const current = stats[name] || (stats[name] = { total: 0, pass: 0, fail: 0 });
      current.total++;
      if (entry.result.passed && !entry.result.skipped && !entry.result.todo) current.pass++;
      else if (!entry.result.skipped && !entry.result.todo) current.fail++;
    });
    return stats;
  }
  ...
  summaryDisplay() {
    ...
    if (this.includeLauncherProperties) {
      ...
      Object.keys(stats).forEach(launcher => {
        const property = stats[launcher];
        ['pass', 'fail'].forEach(status => {
          const node = doc.createElement('property');
          node.setAttribute('name', `${launcher}_${status}`);
          node.setAttribute('value', `${property[status]}`);
          properties.appendChild(node);
        });
      });
      const launcher = doc.createElement('property');
      launcher.setAttribute('name', 'launcher');
      launcher.setAttribute('value', this.launcherName || '');
      properties.appendChild(launcher);
      const launchers = doc.createElement('property');
      launchers.setAttribute('name', 'launchers');
      launchers.setAttribute('value', Object.keys(stats).join(','));
      properties.appendChild(launchers);
