...
module.exports = class XUnitReporter {
  ...
  getLauncherStats() {
    ...
    this.results.forEach(item => {
      const name = item.launcher;
      stats[name] = stats[name] || { total: 0, pass: 0, fail: 0 };
      const stat = stats[name];
      stat.total++;
      if (item.result.passed && !item.result.skipped && !item.result.todo) stat.pass++;
      else if (!item.result.skipped && !item.result.todo) stat.fail++;
    });
    return stats;
  }
