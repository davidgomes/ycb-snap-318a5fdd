...
module.exports = class XUnitReporter {
  ...
  summaryDisplay() {
    ...
    if (this.bailed) {
      ...
      var bailProperties = [
        ['bailReason', this.bailReason === null || this.bailReason === undefined ? '' : String(this.bailReason)],
        ['testsBeforeBail', String(this.testsRanBeforeBail)],
        ['suppressedAfterBail', String(this.suppressedAfterBail)]
      ];
      for (var p = 0; p < bailProperties.length; p++) {
        var propertyNode = doc.createElement('property');
        propertyNode.setAttribute('name', bailProperties[p][0]);
        propertyNode.setAttribute('value', bailProperties[p][1]);
        propertiesNode.appendChild(propertyNode);
      }
      rootNode.appendChild(propertiesNode);

      var bailErrorNode = doc.createElement('error');
      var bailMessage = 'Bail out! ' + (this.bailReason === null || this.bailReason === undefined ? '' : String(this.bailReason));
      bailErrorNode.setAttribute('message', bailMessage);
      bailErrorNode.appendChild(doc.createTextNode(bailMessage));
    ...
    if (this.bailed) {
      ...
      var bailSummary = 'Bail out! ' + (this.bailReason === null || this.bailReason === undefined ? '' : String(this.bailReason)) +
        '\n# ran before bail ' + this.testsRanBeforeBail +
        '\n# suppressed ' + this.suppressedAfterBail;
      systemOut.appendChild(doc.createTextNode(bailSummary));
