...
module.exports = class XUnitReporter {
  constructor(silent, out, config) {
    ...
    this.endTime = null;
    this.bailSummary = null;
  }

  setBailSummary(bailSummary) {
    this.bailSummary = bailSummary;
  }
  ...
  summaryDisplay() {
    ...
    var bailSummary = this.bailSummary;
    if (bailSummary) {
      rootNode.setAttribute('errors', '1');
      rootNode.appendChild(this.getBailPropertiesNode(doc, bailSummary));

      var bailErrorNode = doc.createElement('error');
      bailErrorNode.setAttribute('message', displayutils.bailOutLine(bailSummary));
    ...
    if (bailSummary) {
      ...
      systemOutNode.appendChild(doc.createCDATASection([
        displayutils.bailOutLine(bailSummary),
        'bailed',
        `ran before bail ${bailSummary.testsRanBeforeBail}`,
        `suppressed ${bailSummary.suppressedAfterBail}`
      ].join('\n')));
  ...
  getBailPropertiesNode(document, bailSummary) {
    var propertiesNode = document.createElement('properties');
    [
      ['bailReason', bailSummary.reason],
      ['testsBeforeBail', bailSummary.testsRanBeforeBail],
      ['suppressedAfterBail', bailSummary.suppressedAfterBail]
    ].forEach(function(property) {
      var propertyNode = document.createElement('property');
      propertyNode.setAttribute('name', property[0]);
      propertyNode.setAttribute('value', `${property[1]}`);
