...
module.exports = class XUnitReporter {
  ...
  summaryDisplay() {
    ...
    var bailInfo = this.bailInfo;
    if (bailInfo) {
      rootNode.setAttribute('errors', '1');
      var propertiesNode = doc.createElement('properties');
      [
        ['bailReason', bailInfo.reason],
        ['testsBeforeBail', bailInfo.testsRanBeforeBail],
        ['suppressedAfterBail', bailInfo.suppressedAfterBail]
      ].forEach(function(pair) {
        var propertyNode = doc.createElement('property');
        propertyNode.setAttribute('name', pair[0]);
        propertyNode.setAttribute('value', `${pair[1]}`);
    ...
    if (bailInfo) {
      ...
      bailErrorNode.setAttribute('message', `Bail out! ${bailInfo.reason}`);
      bailErrorNode.setAttribute('type', 'bail');
      ...
      systemOutNode.appendChild(doc.createCDATASection(
        `Bail out! ${bailInfo.reason}\n` +
        `Tests ran before bail: ${bailInfo.testsRanBeforeBail}\n` +
        `Suppressed after bail: ${bailInfo.suppressedAfterBail}`
      ));
