...
module.exports = class XUnitReporter {
  ...
  getLauncherStats() {
    ...
    this.results.forEach(entry => {
      ...
      if (!stats[name]) {
        stats[name] = { total: 0, pass: 0, fail: 0 };
      }
      const r = entry.result;
      stats[name].total++;
      if (r.passed && !r.todo) {
        stats[name].pass++;
      } else if (!r.passed && !r.skipped && !r.todo) {
        stats[name].fail++;
      }
    });
    return stats;
  ...
  getPropertiesNode(doc) {
    ...
    addProperty('launchers', names.join(','));
    names.forEach(function(name) {
      addProperty(`${name}_pass`, stats[name].pass);
      addProperty(`${name}_fail`, stats[name].fail);
    });
