...
func StartMonitoring(targets []config.Target, options Options) {
	...
	for _, key := range allKeys {
		...
		trackers[key.String()] = alerts.NewTracker(targets[key.TargetIndex].AlertPolicy.ToAlertsPolicy())
	}
	...
	for i, target := range targets {
		...
		go func(t config.Target, index int) {
			defer wg.Done()
			monitorTargetTUI(ctx, t, index, monitors, sequences, trackers, dataChannel, options)
		}(target, i)
	}
...
func monitorTargetTUI(ctx context.Context, target config.Target, targetIndex int, monitors map[string]*stats.Monitor, sequences map[string]*int, trackers map[string]*alerts.Tracker, dataChannel chan<- TargetData, options Options) {
			...
			for _, lambdaResult := range lambdaResults {
				...
				if monitor, exists := monitors[targetKeyStr]; exists {
					...
					if target.ReceiveAlert {
						if err := notifications.HandleAlertDecision(decision, target.Name, lambdaResult.Result.URL); err != nil {
							dataChannel <- TargetData{
								Target:     target,
