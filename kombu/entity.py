...
class Queue(MaybeChannelBound):
    ...
    def is_single_active_consumer(self):
        """Return :const:`True` if declared with ``x-single-active-consumer``."""
        return bool((self.queue_arguments or {}).get(
            'x-single-active-consumer'))

    @property
    def consumer_priority(self):
        """Consumer priority from ``x-priority`` (defaults to 0)."""
        try:
            return int((self.consumer_arguments or {}).get('x-priority') or 0)
        except (TypeError, ValueError):
            return 0

    @classmethod
    def with_consumer_priority(cls, name, exchange, priority=0, **kwargs):
        """Create queue whose consumers use the given ``x-priority``."""
    ...
    def with_single_active_consumer(cls, name, exchange, durable=True,
                                    **kwargs):
        """Create queue declared with ``x-single-active-consumer``."""
