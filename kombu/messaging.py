...
class Consumer:
    ...
    #: List of callbacks called with the consumer tag when one of the
    #: queue consumers is cancelled.
    #:
    #: Only queue consumers started while at least one callback is
    #: registered notify them, see :meth:`on_cancel_notify`.
    ...
    def on_cancel_notify(self, callback):
        """Add callback called with the consumer tag on consumer cancel.

        Returns
        -------
            Consumer: this consumer, to allow chaining.
        """
    ...
    def consuming_from_sac(self, queue):
        """Return :const:`True` if consuming from single active consumer queue.
        """
        ...
        return declared is not None and declared.is_single_active_consumer

    def is_active_on(self, queue):
        """Return :const:`True` if holding the active consumer tag of queue.

        Always true while consuming from the queue if the channel does
        not track active consumers.
        """
        name = queue.name if isinstance(queue, Queue) else queue
        tag = self._active_tags.get(name)
        if tag is None:
            return False
        get_active_consumer = getattr(self.channel, 'get_active_consumer', None)
        if get_active_consumer is None:
            return True
        return get_active_consumer(name) == tag

    @property
    def active_consumer_tags(self):
        """Tags of the queue consumers started by this consumer."""
    ...
    def _basic_consume(self, queue, consumer_tag=None,
        ...
        if tag is None:
            ...
            # AMQP transports raise ConsumerCancelled on broker initiated
            # cancels only when no on_cancel callback was given.
            options = ({'on_cancel': self._on_cancel}
                       if self.cancel_notify_callbacks else {})
            queue.consume(tag, self._receive_callback,
                          no_ack=no_ack, nowait=nowait, **options)
        return tag
