		...
		if len(regions) > 0 {
			...
			for _, lambdaResult := range lambdaResults {
				...
				if monitor, exists := monitors[keyStr]; exists {
					...
					decision := evaluateAlert(trackers[keyStr], target, lambdaResult.Result, lambdaResult.Region)
			...
			if monitor, exists := monitors[keyStr]; exists {
				...
				decision := evaluateAlert(trackers[keyStr], target, result, "")
...
func evaluateAlert(tracker *alerts.Tracker, target config.Target, result net.WebsiteCheckResult, region string) alerts.Decision {
	if tracker == nil {
	...
	if target.WebhookURL != "" {
		if err := notifications.HandleWebhookDecisionWithHeaders(target.WebhookURL, target.WebhookHeaders, decision, target.Name, result.URL, result.ResponseTime, result.StatusCode, getErrorMessage(result), region); err != nil {
