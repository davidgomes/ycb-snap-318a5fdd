		...
		if len(regions) > 0 {
			...
			for _, lambdaResult := range lambdaResults {
				...
				if monitor, exists := monitors[keyStr]; exists {
					...
					decision := observeAlert(trackers, keyStr, target, lambdaResult.Result, lambdaResult.Region)
			...
			if monitor, exists := monitors[keyStr]; exists {
				...
				decision := observeAlert(trackers, keyStr, target, result, "")
...
func observeAlert(trackers map[string]*alerts.Tracker, key string, target config.Target, result net.WebsiteCheckResult, region string) alerts.Decision {
