		...
		if len(regions) > 0 {
			...
			for _, lambdaResult := range lambdaResults {
				...
				if monitor, exists := monitors[keyStr]; exists {
					...
					decision := evaluateTargetAlert(trackers, keyStr, target, lambdaResult.Result, lambdaResult.Result.URL)
			...
			if monitor, exists := monitors[keyStr]; exists {
				...
				decision := evaluateTargetAlert(trackers, keyStr, target, result, target.URL)
				if target.WebhookURL != "" {
...
func policyFromConfig(p config.AlertPolicy) alerts.Policy {
...
func evaluateTargetAlert(trackers map[string]*alerts.Tracker, key string, target config.Target, result net.WebsiteCheckResult, rawURL string) alerts.Decision {
	tracker, ok := trackers[key]
	if !ok {
		tracker = alerts.NewTracker(policyFromConfig(target.AlertPolicy))
		trackers[key] = tracker
	}
	sslDays := -1
	if strings.HasPrefix(rawURL, "https://") {
		sslDays = net.GetSSLCertExpiry(rawURL)
	}
