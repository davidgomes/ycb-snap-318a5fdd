...
func StartMultiTargetMonitoring(targets []config.Target, options MonitoringOptions) {
	...
	for _, key := range allKeys {
		...
		trackers[keyStr] = alerts.NewTracker(targets[key.TargetIndex].AlertPolicy.ToAlertsPolicy())
	}
	...
	for i, target := range targets {
		...
		go func(t config.Target, index int) {
			defer wg.Done()
			monitorTargetSimple(ctx, t, index, monitors, sequences, trackers, resultsChan, options)
		}(target, i)
	}
...
func evaluateAlert(tracker *alerts.Tracker, target config.Target, result net.WebsiteCheckResult, sslDays int, region string) alerts.Decision {
	...
	if target.ReceiveAlert {
		if err := notifications.HandleAlertDecision(decision, target.Name, target.URL); err != nil {
	...
	if target.WebhookURL != "" {
		...
		if err := notifications.HandleWebhookDecisionWithHeaders(target.WebhookURL, target.WebhookHeaders, decision, target.Name, target.URL, result.ResponseTime, result.StatusCode, errorMsg, region); err != nil {
...
func sslDaysForPolicy(target config.Target) int {
...
func monitorTargetSimple(ctx context.Context, target config.Target, targetIndex int, monitors map[string]*stats.Monitor, sequences map[string]*int, trackers map[string]*alerts.Tracker, resultsChan chan<- TargetResult, options MonitoringOptions) {
	...
	makeRequest := func() {
		sslDays := sslDaysForPolicy(target)
		attemptCount++
		...
		if len(regions) > 0 {
			...
			for _, lambdaResult := range lambdaResults {
				...
				if monitor, exists := monitors[keyStr]; exists {
					...
					if tracker, exists := trackers[keyStr]; exists {
						decision = evaluateAlert(tracker, target, lambdaResult.Result, sslDays, lambdaResult.Region)
					}
			...
			if monitor, exists := monitors[keyStr]; exists {
				...
				var decision alerts.Decision
				if tracker, exists := trackers[keyStr]; exists {
					decision = evaluateAlert(tracker, target, result, sslDays, "")
				}
