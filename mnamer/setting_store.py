...
class SettingStore:
    ...
    daemon: str | None = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            flags=["--daemon"],
            group=SettingType.DIRECTIVE,
            help="--daemon={start,stop,status,logs,stats,restart}: control the media daemon",
        ).as_dict(),
    )
    daemon_run_once: bool = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--daemon-run-once: scan watches once and exit",
        ).as_dict(),
    )
    dry_run: bool = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--dry-run: report moves without changing files, state, or logs",
        ).as_dict(),
    )
    validate_daemon_config: bool = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--validate-daemon-config: check --daemon-config JSON and exit",
        ).as_dict(),
    )
    ...
    watch: list[str] = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            flags=["--watch"],
            group=SettingType.DIRECTIVE,
            help="--watch=<PATH,...>: top-level directories to scan; combined with targets",
            nargs="+",
        ).as_dict(),
    )
    ...
    stability_checks: int = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--stability-checks=<COUNT>: skip a file if its size changes",
            typevar=int,
        ).as_dict(),
    )
    batch_size: int | None = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--batch-size=<NUMBER>: max files moved per cycle across all watches (0 moves none)",
            typevar=int,
        ).as_dict(),
    )
    lines: int | None = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            flags=["--lines"],
            group=SettingType.DIRECTIVE,
            help="--lines=<NUMBER>: with --daemon logs, print the last N lines",
            typevar=int,
        ).as_dict(),
    )
    notify_webhook: str | None = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--notify-webhook=<URL>: POST after moves; failures are ignored",
        ).as_dict(),
    )
    ...
    def bulk_apply(self, d: dict[str, Any]):
        for k, v in d.items():
            # Integer 0 is a real setting (for example --batch-size 0).
            if v or v == 0:
                setattr(self, k, v)
