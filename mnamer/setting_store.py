...
class SettingStore:
    ...
    daemon: str | None = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            flags=["--daemon"],
            group=SettingType.DIRECTIVE,
            help="--daemon={start,stop,status,logs,stats,restart}: control daemon",
        ).as_dict(),
    )
    daemon_run_once: bool = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--daemon-run-once: run a single daemon processing cycle",
        ).as_dict(),
    )
    ...
    validate_daemon_config: bool = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--validate-daemon-config: validate --daemon-config then exit",
        ).as_dict(),
    )
    daemon_config: str | None = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--daemon-config=<PATH>: daemon watch configuration JSON",
        ).as_dict(),
    )
    ...
    watch: list[str] = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            flags=["--watch"],
            group=SettingType.DIRECTIVE,
            help="--watch=<PATH ...>: directories for the daemon to watch",
            nargs="+",
        ).as_dict(),
    )
    stability_interval_ms: int = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--stability-interval-ms=<MS>: poll interval between size checks",
            typevar=int,
        ).as_dict(),
    )
    stability_checks: int = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--stability-checks=<COUNT>: number of file size checks",
            typevar=int,
        ).as_dict(),
    )
    batch_size: int | None = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--batch-size=<N>: max files moved per daemon cycle",
            typevar=int,
        ).as_dict(),
    )
    lines: int | None = dataclasses.field(
        ...
        metadata=SettingSpec(
            flags=["--lines"],
            group=SettingType.DIRECTIVE,
            help="--lines=<N>: number of trailing daemon log lines to show",
            typevar=int,
        ).as_dict(),
    )
    notify_webhook: str | None = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--notify-webhook=<URL>: POST daemon moves to this URL",
        ).as_dict(),
    )
    ...
    def bulk_apply(self, d: dict[str, Any]):
        for k, v in d.items():
            if v or (type(v) is int and k in {"batch_size", "lines"}):
                setattr(self, k, v)
