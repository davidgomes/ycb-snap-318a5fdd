...
class SettingStore:
    ...
    dry_run: bool = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--dry-run: report daemon moves without changing files",
        ).as_dict(),
    )
    ...
    watch: list[str] = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            flags=["--watch"],
            group=SettingType.DIRECTIVE,
            help="--watch=<PATH,...>: directories to scan (non-recursive)",
            nargs="+",
        ).as_dict(),
    )
    ...
    stability_checks: int = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--stability-checks=<N>: size samples required before a move",
            typevar=int,
        ).as_dict(),
    )
    batch_size: int | None = dataclasses.field(
        ...
        metadata=SettingSpec(
            ...
            group=SettingType.DIRECTIVE,
            help="--batch-size=<N>: max files to move per cycle (0 moves none)",
            typevar=int,
        ).as_dict(),
    )
    ...
    def bulk_apply(self, d: dict[str, Any]):
        for k, v in d.items():
            # Keep falsy values like False and [] from clobbering defaults, but
            # apply numeric 0 so flags such as --batch-size 0 take effect.
