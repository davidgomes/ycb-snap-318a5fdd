...
For now, sqlfmt only works on `select`, `delete`, `grant`, `revoke`, `create function`, and `create table` (with column definitions) statements (which is all you need if you use sqlfmt with a dbt project). It is being extended to additional DDL and DML. Visit [this tracking issue](https://github.com/tconbeer/sqlfmt/issues/262) for more information.
