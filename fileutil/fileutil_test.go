...
func TestEnsureFileName(t *testing.T) {
	...
	enc := EnsureFileName("/Users/jack/Desktop/hello.sql", true, true, false)
	assert.Equal(t, "/Users/jack/Desktop/hello.sql.gz.enc", enc)
}
...
func TestEnsureFileSuffix(t *testing.T) {
	...
	f = EnsureFileSuffix("test.sql.enc", true, true)
	assert.Equal("test.sql.gz.enc", f)
