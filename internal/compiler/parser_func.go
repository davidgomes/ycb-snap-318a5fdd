...
func (p *parsing) parseFunc(tok token, kind funcKindToParse) (ast.Node, token) {
	...
	tok = p.next()
	if !isMacro && kind&parseFuncDecl != 0 && tok.typ == tokenLeftParenthesis {
		var params []*ast.Parameter
		var recvVariadic bool
		params, recvVariadic, _, tok = p.parseFuncParameters(tok, false, false)
		if recvVariadic || len(params) > 1 {
