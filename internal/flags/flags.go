...
func init() {
	...
	pflag.BoolVar(&Graph, "graph", false, "Displays the task dependency graph.")
	pflag.StringVar(&GraphFormat, "format", "", "Graph output format: [json|dot|text].")
	pflag.BoolVar(&GraphReverse, "reverse", false, "Show tasks that depend on the given task.")
...
func Validate() error {
	...
	if ListJson && !List && !ListAll && !Graph {
