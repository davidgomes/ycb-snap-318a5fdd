...
func init() {
	...
	pflag.BoolVar(&NoStatus, "no-status", false, "Ignore status when listing tasks as JSON or graph")
	pflag.BoolVar(&Nested, "nested", false, "Nest namespaces when listing tasks as JSON")
	pflag.BoolVar(&Graph, "graph", false, "Show the task dependency graph")
	pflag.StringVar(&GraphFormat, "format", "json", "Graph format: [json|dot|text]")
	pflag.BoolVar(&GraphReverse, "reverse", false, "Show tasks that depend on the given tasks")
...
func Validate() error {
	...
	if GraphFormat != "json" && GraphFormat != "dot" && GraphFormat != "text" {
		return errors.New("task: --format must be one of json, dot, or text")
	}
	if Graph && (List || ListAll || ListJson || Nested) {
		return errors.New("task: --graph cannot be combined with task listing flags")
	}
	if GraphReverse && !Graph {
		return errors.New("task: --reverse only applies to --graph")
	}
	if NoStatus && !Graph && !ListJson {
		return errors.New("task: --no-status only applies to --json with --list or --graph")
	}
