...
func init() {
	...
	pflag.BoolVar(&Graph, "graph", false, "Print the dependency graph of the given tasks.")
	pflag.StringVar(&GraphFormat, "format", "", "Output format for --graph: [json|dot|text]. Defaults to json.")
	pflag.BoolVar(&GraphReverse, "reverse", false, "With --graph, show tasks that depend on the given tasks.")
...
func Validate() error {
	...
	if Graph && (List || ListAll) {
		return errors.New("task: cannot use --graph together with --list or --list-all")
	}
	...
	case "", "json", "dot", "text":
	default:
		return fmt.Errorf("task: unknown graph format %q", GraphFormat)
	}
