...
class StencilFunc(object):
    ...
    def add_indices_to_kernel(self, kernel, index_names, ndim,
        ...
        const_dict = {}
        kernel_consts = []
        modes = self._mode if isinstance(self._mode, tuple) else (self._mode,) * ndim
                ...
                if (isinstance(stmt, ir.Assign)
                    ...
                    if ndim == 1:
                        ...
                        if isinstance(stmt_index_var_typ, types.misc.SliceType):
                            ...
                            access = ir.Expr.getitem(stmt.value.value, tmpvar, loc)
                        ...
                        new_body.append(ir.Assign(tuple_call, s_index_var, loc))
                        access = ir.Expr.getitem(stmt.value.value, s_index_var, loc)
                        new_body.append(ir.Assign(access, stmt.target, loc))
                else:
                    new_body.append(stmt)
            block.body = new_body
    ...
    def _stencil_wrapper(self, result, sigret, return_type, typemap, calltypes, *args):
        ...
        if isinstance(modes, tuple):
            ...
            mode_by_dim = (modes,) * the_array.ndim
