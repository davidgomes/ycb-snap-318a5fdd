...
It extends ``FailableN`` directly and not ``DiverseFailableN``,
because ``Validated.swap`` wraps values into a tuple of errors,
so ``x.swap().swap() == x`` does not hold.
"""
...
class ValidatedLikeN(
    ...
    """Base type for error-accumulating containers."""
    ...
    def from_failure(
        ...
        """Unit method to create failed containers from a single error."""
    ...
    def from_result(
        ...
        """Creates a new container from ``Result``."""
...
class ValidatedBasedN(
    ...
    unwrappable.Unwrappable[_FirstType, _SecondType],
    equable.Equable,
):
    """Base type for real ``Validated`` types. Can be unwrapped."""
