...
and ``x.swap().swap() == x`` does not hold.
"""
...
class ValidatedLikeN(
    ...
    """
    Base type for types that look like ``Validated``.

    Its failed state accumulates errors when ``.apply`` is used,
    ...
    def from_failure(
        ...
        """Unit method to create new failed containers from a single error."""
    ...
    def from_result(
        ...
        """Creates new containers from ``Result`` values."""
    ...
    def from_validated(
        ...
        """Creates new containers from ``Validated`` values."""
...
class UnwrappableValidated(
    ...
    """
    Intermediate type with 5 type arguments for unwrappable validated types.
...
class ValidatedBasedN(
    ...
    Can be unwrapped, failures are unwrapped as a tuple of errors.
    """
