...
``ValidatedLikeN`` cannot extend
:class:`returns.interfaces.failable.DiverseFailableN`.
That interface requires :class:`returns.interfaces.swappable.SwappableN`,
whose ``double_swap_law`` says ``x.swap().swap() == x``.
``Validated`` wraps a success value in a one-element tuple when swapping,
so swapping twice is not the identity.
"""
...
class _ValidatedLawSpec(LawSpecDef):
    """
    Short-circuit laws for failed ``Validated`` values.

    ``.map``, ``.bind``, and ``.apply`` must not run the success function
    when the container is already a failure.
    Error accumulation is an applicative concern and is not covered here.
    """
    ...
    def apply_short_circuit_law(
        ...
        """Ensures that applying a success function keeps a failure."""
...
class ValidatedLikeN(
    ...
    Failures store an error element type and accumulate through ``.apply``.
    ...
    def from_failure(
        ...
        """Creates a failed container from a single error value."""
    ...
    def from_result(
        ...
        """Creates a new container from a ``Result``."""
    ...
    def from_validated(
        ...
        """Creates a new container from an existing ``Validated``."""
...
class UnwrappableValidated(
    ...
    """
    Intermediate type with 5 type arguments for unwrappable validation.
...
class ValidatedBasedN(
    ...
    Successful containers unwrap to the value.
    Failed containers unwrap to a tuple of error elements.
    """
