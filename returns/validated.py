...
class Validated(  # type: ignore[type-var]
    ...
    """
    Base class for :class:`~Invalid` and :class:`~Valid`.
    ...
    def swap(self) -> 'Validated[tuple[_ErrorType_co, ...], _ValueType_co]':
        ...
        Values become a single error, errors become a value.
        Note, that ``.swap().swap()`` does not return the original container.
    ...
    def apply(
        ...
        Errors of both failed containers are accumulated:
        errors of this container come first.
    ...
    def lash(  # type: ignore[override]
        ...
        The function receives a tuple of all errors.

        .. code:: python

          >>> from returns.validated import Validated, Valid, Invalid

          >>> def lashable(errors: tuple[str, ...]) -> Validated[str, str]:
          ...      if len(errors) > 1:
          ...
          >>> assert Invalid(('a', 'b')).lash(lashable) == Valid('ab')
    ...
    def do(
        ...
        Short-circuits on the first failure, like ``.bind`` does.
    ...
    def from_failure(
        ...
        The error is wrapped into a tuple, so accumulation works uniformly.
    ...
    def from_result(
        ...
        ``Failure`` error is wrapped into a tuple with a single error.
    ...
    def combine(
        ...
        Errors from both containers are accumulated, left to right.
    ...
    def combine_n(
        ...
        Errors from all containers are accumulated, left to right.
        ...
        .. code:: python
          ...
          ...     (Valid(1), Valid(2), Valid(3)), add,
          ... ) == Valid(6)
          >>> assert Validated.combine_n(
          ...     (Invalid(('a',)), Valid(2), Invalid(('b', 'c'))), add,
          ... ) == Invalid(('a', 'b', 'c'))
...
class Invalid(Validated[Any, _ErrorType_co]):  # noqa: WPS338
    ...
    Stores all accumulated errors as an immutable tuple.
    """
    ...
    def __init__(self, inner_value: Iterable[_ErrorType_co]) -> None:
        """Invalid constructor."""
    ...
    if not TYPE_CHECKING:  # noqa: WPS604  # pragma: no branch
        ...
        def alt(self, function):
            ...
            return Invalid(function(error) for error in self._inner_value)
        ...
        def apply(self, container):
            """Accumulates errors when ``container`` is ``Invalid`` too."""
...
def validated(  # noqa: WPS234
    ...
    Caught exception is stored as a single error inside :class:`~Invalid`.
