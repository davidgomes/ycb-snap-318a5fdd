...
// When encryptor is set, each pipe is wrapped so bytes are gzipped (if requested)
// and then encrypted. Closers run gzip, then encryption, then the pipe writer.
func storageReadWriteCloser(count int, compress bool, encryptor *encryption.Encryptor) ([]io.Reader, io.Writer, io.Closer) {
	...
	for i := 0; i < count; i++ {
		...
		// Encryption must close after gzip so the gzip trailer is encrypted,
		// and the pipe writer must close last so readers see EOF.
		if ew != nil {
			pcs = append(pcs, ew)
		}
		pcs = append(pcs, pw)
...
func (handler *JobHandler) save() error {
	...
	// Resolve the key before any storage or dump work so a bad key fails fast,
	// including when the job has no storages configured.
