...
// When encryptor is set, plaintext is gzip-compressed (if compress is set) and then encrypted.
// Closers run outermost first so gzip finishes before the encryptor trailer and the pipe EOF.
func storageReadWriteCloser(count int, compress bool, encryptor *encryption.Encryptor) ([]io.Reader, io.Writer, io.Closer) {
	...
	for i := 0; i < count; i++ {
		...
		w := io.Writer(pw)
		var layered []io.Closer

		if encryptor != nil {
			ew := encryptor.EncryptWriter(pw)
			w = ew
			layered = append(layered, ew)
		}

		if compress {
			gw := gzip.NewWriter(w)
			w = gw
			layered = append([]io.Closer{gw}, layered...)
		}

		pws = append(pws, w)

		// Close the layered writers before the pipe so the reader observes a finished stream.
		pcs = append(pcs, layered...)
		pcs = append(pcs, pw)
	}
