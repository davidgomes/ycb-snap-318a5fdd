...
// When encryptor is set, each storage stream is gzip-compressed (if requested)
// and then encrypted, so readers recover plaintext with DecryptReader followed by gzip.NewReader.
