...
// Content is gzipped first (if compress is set) and then encrypted (if encryptor is not nil).
func storageReadWriteCloser(count int, compress bool, encryptor *encryption.Encryptor) ([]io.Reader, io.Writer, io.Closer) {
	...
	for i := 0; i < count; i++ {
		...
		// Closers must run from the outermost writer inwards so that each layer flushes into the next one before it is closed.
		for j := len(closers) - 1; j >= 0; j-- {
			pcs = append(pcs, closers[j])
		}

		// The pipe writer must be closed last, after every writer wrapping it has been flushed.
		pcs = append(pcs, pw)
	}
...
// Load the job's encryption key and create an encryptor from it.
func (handler *JobHandler) getEncryptor() (*encryption.Encryptor, error) {
	cfg := handler.Job.Encryption
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create encryptor with the loaded key: %w", err)
	}
