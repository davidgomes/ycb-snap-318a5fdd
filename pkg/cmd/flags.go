...
func addValueOptionsFlags(f *pflag.FlagSet, v *values.Options) {
	...
	f.StringArrayVar(&v.MergeStrategies, "merge-strategy", []string{}, "override a chart array merge strategy (path=append|merge). Chart annotations are used otherwise")
	f.StringArrayVar(&v.MergeKeys, "merge-key", []string{}, "override the merge key for an array path (path=field). The field may be a dotted path")
}
