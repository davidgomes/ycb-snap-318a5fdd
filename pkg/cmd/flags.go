...
func addValueOptionsFlags(f *pflag.FlagSet, v *values.Options) {
	...
	f.StringArrayVar(&v.MergeStrategies, "merge-strategy", []string{}, "override a chart array merge strategy (path=append|merge). Takes precedence over Chart.yaml annotations for the same path")
	f.StringArrayVar(&v.MergeKeys, "merge-key", []string{}, "override a chart array merge key (path=field). Takes precedence over Chart.yaml annotations for the same path")
}
