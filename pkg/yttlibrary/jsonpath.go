...
// Query is a core.StarlarkFunc that returns a list of every value within a document that matches a JSONPath expression.
func (m jsonPathModule) Query(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	doc, path, err := m.queryArgs(args, kwargs)
	if err != nil {
...
// QueryOne is a core.StarlarkFunc that returns the first value within a document that matches a JSONPath expression,
// or None if nothing matches.
func (m jsonPathModule) QueryOne(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	doc, path, err := m.queryArgs(args, kwargs)
	if err != nil {
		return starlark.None, err
	}
...
func (jsonPathModule) queryArgs(args starlark.Tuple, kwargs []starlark.Tuple) (interface{}, string, error) {
	if args.Len() != 2 {
