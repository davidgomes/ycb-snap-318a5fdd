...
// returns the unified manifest stream of all rendered documents, hooks
// included, ordered by Source path and then by position within each template.
func (cfg *Configuration) renderResourcesWithStream(ch *chart.Chart, values common.Values, releaseName, outputDir string, subNotes, useReleaseName, includeCrds bool, pr postrenderer.PostRenderer, interactWithRemote, enableDNS, hideSecret bool) ([]*release.Hook, *bytes.Buffer, string, string, error) {
	...
	// removed here.
	hs, manifests, streamDocs, err := releaseutil.SortManifestsWithStream(files, nil, releaseutil.InstallOrder)
	if err != nil {
	...
	if includeCrds {
		for _, crd := range ch.CRDObjects() {
			streamDocs = append(streamDocs, releaseutil.StreamDocument{Source: crd.Filename, Content: strings.TrimSpace(string(crd.File.Data))})
		}
	}
	if hideSecret {
		for i, d := range streamDocs {
			if d.Hook == nil && d.Head != nil && d.Head.Kind == "Secret" && d.Head.Version == "v1" {
				streamDocs[i].Content = "# HIDDEN: The Secret output has been suppressed"
			}
		}
	}

	return hs, b, notes, releaseutil.FormatManifestStream(streamDocs), nil
}
