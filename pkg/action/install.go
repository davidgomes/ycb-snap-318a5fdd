...
func (i *Install) RunWithContext(ctx context.Context, ch ci.Charter, vals map[string]any) (ri.Releaser, error) {
	...
	var manifestStream string
	rel.Hooks, manifestDoc, rel.Info.Notes, manifestStream, err = i.cfg.renderResourcesWithStream(chrt, valuesToRender, i.ReleaseName, i.OutputDir, i.SubNotes, i.UseReleaseName, i.IncludeCRDs, i.PostRenderer, interactWithServer(i.DryRunStrategy), i.EnableDNS, i.HideSecret)
	...
	if isDryRun(i.DryRunStrategy) {
		...
		rel.ManifestStream = manifestStream
		return rel, nil
	}
