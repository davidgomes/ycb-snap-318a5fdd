...
func (u *Upgrade) prepareUpgrade(name string, chart *chartv2.Chart, vals map[string]any) (*release.Release, *release.Release, bool, error) {
	...
	hooks, manifestDoc, notesTxt, manifestStream, err := u.cfg.renderResourcesWithStream(chart, valuesToRender, "", "", u.SubNotes, false, false, u.PostRenderer, interactWithServer(u.DryRunStrategy), u.EnableDNS, u.HideSecret)
	if err != nil {
		return nil, nil, false, err
	}
	...
	if isDryRun(u.DryRunStrategy) {
		upgradedRelease.ManifestStream = manifestStream
	}
