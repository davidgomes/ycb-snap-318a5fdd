...
func SendWebhook(webhookURL string, headers map[string]string, payload WebhookPayload) error {
	...
	return sendWebhook(client, webhookURL, headers, payload)
}
...
func HandleWebhookDecision(url string, client *http.Client, decision alerts.Decision, name string, urlStr string, respTime time.Duration, status int, errStr string, region string) error {
	return deliverDecision(url, client, nil, decision, name, urlStr, respTime, status, errStr, region)
}
...
func HandleWebhookDecisionWithHeaders(url string, headers []string, decision alerts.Decision, name string, urlStr string, respTime time.Duration, status int, errStr string, region string) error {
	return deliverDecision(url, nil, parseHeaders(headers), decision, name, urlStr, respTime, status, errStr, region)
}

func deliverDecision(url string, client *http.Client, headers map[string]string, decision alerts.Decision, name string, urlStr string, respTime time.Duration, status int, errStr string, region string) error {
	...
	if err := sendWebhook(client, url, headers, payload); err != nil {
		displayName := name
		if displayName == "" {
			displayName = urlStr
		}
