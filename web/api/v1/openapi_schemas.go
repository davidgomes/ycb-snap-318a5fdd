...
func (*OpenAPIBuilder) statusReloadDataSchema() *base.SchemaProxy {
	...
	props.Set("error_message", stringSchemaWithDescription("Error from the most recent reload attempt, if any."))
	props.Set("applied_reloaders", stringArraySchemaWithDescription("Reloaders that applied the new configuration before success or failure."))
	...
	props.Set("rollback_successful", base.CreateSchemaProxy(&base.Schema{Type: []string{"boolean"}, Description: "Whether rollback completed without error."}))
	props.Set("failed_reloader", stringSchemaWithDescription("Name of the reloader that failed, if any."))
	props.Set("reloader_timings_ms", base.CreateSchemaProxy(&base.Schema{
		Type:                 []string{"object"},
		Description:          "Apply duration in milliseconds keyed by reloader name.",
		AdditionalProperties: &base.DynamicValue[*base.SchemaProxy, bool]{A: integerSchema()},
	}))
