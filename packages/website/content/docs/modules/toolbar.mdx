...
The same toolbar container can be shared by several editors. Toolbar actions apply to the editor that most recently had focus or a selection, and the toolbar reflects that editor's formats. While that editor is disabled or read-only, the shared controls are disabled.

```javascript
const first = new Quill('#first-editor', { modules: { toolbar: '#toolbar' } });
const second = new Quill('#second-editor', { modules: { toolbar: '#toolbar' } });
```
