...
import { notifyPairEvent } from '../query/utils/pair-tracking';
...
export function addRelationTarget(
    ...
    notifyPairEvent(world, entity, baseTrait, target, 'add');
...
export function removeRelationTarget(
    ...
    if (removedIndex !== -1) {
        updateQueriesForRelationChange(world, relation, entity);
        notifyPairEvent(world, entity, relationTrait, target, 'remove');
    }
