...
/** Flush deferred commands for this entity, then record a pre-mutation snapshot. */
function beginMutation(world: World, entity: Entity): boolean {
    const ctx = world[$internal];
    ctx.deferredViewDirty = true;
    const existed = ctx.entityTraits.has(entity);
    if (!ctx.applyingDeferred) ctx.beforeMutation?.(entity);
    // A deferred destroy may have removed the entity before this immediate write.
    if (existed && !ctx.entityTraits.has(entity)) return false;
    if (existed && !isEntityAlive(ctx.entityIndex, entity)) return false;
    ctx.captureSnapshot?.(entity);
    return true;
}

function subscriptionsOpen(world: World): boolean {
    return !world[$internal].suppressSubscriptions;
}

export function addTrait(world: World, entity: Entity, ...traits: ConfigurableTrait[]) {
    if (traits.length > 0 && !beginMutation(world, entity)) return;
...
/* @inline */ function addRelationPair(world: World, entity: Entity, pair: RelationPair) {
    ...
    if (relationCtx.exclusive) {
        ...
        if (oldTarget !== undefined && oldTarget !== target) {
            ...
            if (instance && subscriptionsOpen(world)) {
...
export function setTrait(
    ...
    if (!beginMutation(world, entity)) return;
