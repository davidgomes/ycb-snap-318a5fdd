...
import { beginPredicateDefer, endPredicateDefer, onPredicateDependencyChange } from './predicate';
import type {
...
export function createQueryResult<T extends QueryParameter[]>(
    ...
    const results = Object.assign(entities, {
        ...
        updateEach(
            ...
            const predicateTraits = getPredicateDependencies(traits);

            // Predicates are re-evaluated once iteration ends so they see committed values.
            ...
            try {
                ...
                if (options.changeDetection === 'auto') {
                    ...
                    for (let i = 0; i < entities.length; i++) {
                        ...
                        schedulePredicateDependencies(world, entity, predicateTraits);
                    }
...
function getPredicateDependencies(traits: Trait[]) {
    const dependencies: Trait[] = [];
    for (let i = 0; i < traits.length; i++) {
        if (traits[i][$internal].predicates.length > 0) dependencies.push(traits[i]);
    }
    return dependencies;
}

function schedulePredicateDependencies(
    world: World,
    entity: Entity,
    predicateTraits: Trait[]
) {
    for (let i = 0; i < predicateTraits.length; i++) {
        onPredicateDependencyChange(world, entity, predicateTraits[i]);
    }
}
