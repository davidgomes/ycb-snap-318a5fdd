...
import { notifyTraitChanged } from '../../aspect/watchers';
...
export function createChanged() {
    ...
    return <T extends readonly (TraitOrRelation | Aspect)[]>(
        ...inputs: T
    ): Modifier<Trait[], `changed-${number}`, NormalizeModifierSources<T>> => {
        const { traits, sources } = splitModifierInputs(inputs);
        return createModifier(`changed-${id}`, id, traits, sources as NormalizeModifierSources<T>);
    };
}
...
export function setChanged(world: World, entity: Entity, trait: Trait) {
    ...
    notifyTraitChanged(world, entity, trait);
}
