...
Number.prototype.changed = function (this: Entity, trait: Trait | RelationPair) {
    const world = getEntityWorld(this);
    if (isRelationPair(trait) && typeof trait[$internal].target === 'number') {
        return setPairChanged(world, this, trait[$internal].relation, trait[$internal].target);
    }
    return setChanged(world, this, trait as Trait);
};
