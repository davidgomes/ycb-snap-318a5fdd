...
Number.prototype.changed = function (this: Entity, trait: Trait | RelationPair) {
    const world = getEntityWorld(this);
    if (!isRelationPair(trait)) return setChanged(world, this, trait);

    const { relation, target } = trait[$internal];
    const relationTrait = relation[$internal].trait;
    const targets = target === '*' ? getRelationTargets(world, relation, this) : [target];
    for (const t of targets) setPairChanged(world, this, relationTrait, t);
};
