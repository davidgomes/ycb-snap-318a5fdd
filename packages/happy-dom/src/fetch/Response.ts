...
export default class Response implements Response {
	...
	public async arrayBuffer(): Promise<ArrayBuffer> {
		...
		if (!buffer) {
			buffer = this.body ? await this.#consumeUnbufferedBody(window) : Buffer.alloc(0);
		}
	...
	public async formData(): Promise<FormData> {
		...
		if (contentType && this.body && /multipart/i.test(contentType)) {
			...
			// Fully buffered multipart bodies stay readable after the page is discarded.
			if (!browserFrame) {
			...
			const taskID = this.#startBodyReadTask(window, asyncTaskManager);
	...
	/**
	 * Consumes a body that is not already buffered.
	 *
	 * Shutdown and navigation abort the read with AbortError.
	 *
	 * @param window Window.
	 * @returns Buffer.
	 */
	async #consumeUnbufferedBody(window: BrowserWindow): Promise<Buffer> {
		const browserFrame = new WindowBrowserContext(window).getBrowserFrame();
		...
		try {
			return await FetchBodyUtility.consumeBodyStream(window, this);
		} catch (error) {
			asyncTaskManager.endTask(taskID);
			throw error;
		} finally {
	 ...
	 * @returns Task ID.
	 */
	#startBodyReadTask(window: BrowserWindow, asyncTaskManager: AsyncTaskManager): number {
		try {
			return asyncTaskManager.startTask(() => {
				this[PropertySymbol.aborted] = true;
				cancelActiveReader(this);
			});
		} catch (error) {
			if (this[PropertySymbol.aborted]) {
				throw new window.DOMException(
					'Failed to read response body: The stream was aborted.',
					DOMExceptionNameEnum.abortError
				);
