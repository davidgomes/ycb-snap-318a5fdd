...
export default class Response implements Response {
	...
	public async arrayBuffer(): Promise<ArrayBuffer> {
		...
		const buffer =
			...
			(await this.#consumeBody(() => FetchBodyUtility.consumeBodyStream(window, this)));
	...
	public async formData(): Promise<FormData> {
		...
		if (contentType && this.body && /multipart/i.test(contentType)) {
			...
			const { formData, buffer } = await this.#consumeBody(() =>
	...
	#consumeBody<T>(consume: () => Promise<T>): Promise<T> {
		return FetchBodyUtility.consumeBodyAsAsyncTask({
			...
			isBuffered: !!this[PropertySymbol.buffer],
			consume
		});
	}
