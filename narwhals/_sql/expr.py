    ...
    def _rolling_window_func(
        ...
        *,
        ...
        def func(
            ...
            if func_name in {"sum", "mean", "min", "max", "median", "quantile"}:
                func_: str = func_name
            ...
            extra_args = () if quantile is None else (quantile,)
            return [
                self._when(
                    self._window_expression(
    ...
    def rolling_quantile(
        ...
        *,
        ...
        if interpolation != "linear":
            msg = (
                f"Only linear interpolation methods are supported for "
                f"`rolling_quantile` with {self._implementation} backend."
            )
            raise NotImplementedError(msg)
        return self._with_window_function(
            self._rolling_window_func(
                "quantile", window_size, min_samples, quantile=quantile, center=center
            )
        )
