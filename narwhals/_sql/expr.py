...
class SQLExpr(LazyExpr[SQLLazyFrameT, NativeExprT], Protocol[SQLLazyFrameT, NativeExprT]):
    ...
    def _rolling_quantile_expr(self, expr: NativeExprT, quantile: float) -> NativeExprT:
        return self._function("quantile", expr, quantile)
        ...
        def func(
            ...
            return [
                self._when(
                    ...
                    >= self._lit(min_samples),
                    self._window_expression(
                        self._rolling_quantile_expr(expr, quantile), **window_kwargs
                    ),
                )
                for expr in self(df)
            ]
