...
class Expr:
    ...
    def rolling_min(
        ...
        Examples:
            >>> import pandas as pd
            >>> import narwhals as nw
            >>> df_native = pd.DataFrame({"a": [3.0, 1.0, None, 4.0]})
            >>> df = nw.from_native(df_native)
            >>> df.with_columns(
    ...
    def rolling_quantile(
        ...
        *,
        ...
        Warning:
            ...
            hence `rolling_quantile` in an `over` context is not available for DuckDB.

        Arguments:
            window_size: The length of the window in number of elements. It must be a
                strictly positive integer.
            quantile: Quantile between 0.0 and 1.0.
            interpolation: Interpolation method to use when the desired quantile lies
                between two data points. One of `'linear'`, `'lower'`, `'higher'`,
                `'nearest'`, `'midpoint'`.
            min_samples: The number of values in the window that should be non-null before
                computing a result. If set to `None` (default), it will be set equal to
                `window_size`. If provided, it must be a strictly positive integer, and
