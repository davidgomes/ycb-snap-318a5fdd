...
class DuckDBExpr(SQLExpr["DuckDBLazyFrame", "Expression"]):
    ...
    def rolling_quantile(
        ...
        *,
        ...
        # `quantile_cont` can be windowed, but `percentile_cont` cannot. Narwhals
        # does not expose rolling quantiles on DuckDB for that reason.
        msg = (
            "`Expr.rolling_quantile` is not supported for the DuckDB backend, "
            "because DuckDB does not support `percentile_cont` as a windowed "
            "aggregate function."
        )
