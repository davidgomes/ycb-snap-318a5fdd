...
func (p AlertPolicy) inherit(global AlertPolicy) AlertPolicy {
	if p.ConsecutiveFailures == 0 {
		p.ConsecutiveFailures = global.ConsecutiveFailures
	}
	if p.ConsecutiveRecoveries == 0 {
		p.ConsecutiveRecoveries = global.ConsecutiveRecoveries
	}
	if p.CooldownSeconds == 0 {
		p.CooldownSeconds = global.CooldownSeconds
	}
	if p.LatencyThresholdMs == 0 {
		p.LatencyThresholdMs = global.LatencyThresholdMs
	}
	if p.LatencyBreachCount == 0 {
		p.LatencyBreachCount = global.LatencyBreachCount
	}
	if p.SSLExpiryThresholdDays == 0 {
		p.SSLExpiryThresholdDays = global.SSLExpiryThresholdDays
	}
	return p
}
