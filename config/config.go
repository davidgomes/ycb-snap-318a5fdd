...
func LoadConfig(configFile string) (*Config, error) {
	...
	normalizeAlertPolicy(&config.Global.AlertPolicy)
	...
	for i := range config.Targets {
		...
		if !viper.IsSet(fmt.Sprintf("targets.%d.alert_policy", i)) {
			target.AlertPolicy = config.Global.AlertPolicy
		} else {
			normalizeAlertPolicy(&target.AlertPolicy)
		}
	}
...
func normalizeAlertPolicy(policy *AlertPolicy) {
