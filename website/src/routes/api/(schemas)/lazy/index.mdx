...
If the recursive shape can be written by placing <Link href="/api/Recur/">`Recur`</Link> inside the schema, <Link href="/api/recursive/">`recursive`</Link> infers the input and output types for you.
