...
If the recursive shape can be written by placing <Link href="/api/Recur/">`Recur`</Link> inside the schema, <Link href="/api/recursiveAsync/">`recursiveAsync`</Link> infers the input and output types for you.
...
<ApiList items={['config', 'getDefault', 'getFallback', 'Recur']} />
