...
from ._multipart import (
    ...
    get_multipart_boundary_from_response_content_type,
)
...
class MultipartPart:
    """
    A single part of a `multipart/*` response body.
    """
...
class Response:
    ...
    def iter_multipart(self) -> typing.Iterator[MultipartPart]:
        ...
        Streaming responses are consumed and closed. Responses that have
        already been read into memory may be iterated repeatedly.
        """
        with request_context(request=self._request):
            content_type = self.headers.get("content-type")
            boundary = get_multipart_boundary_from_response_content_type(content_type)
            ...
            try:
                ...
                if is_streaming and isinstance(self.stream, SyncByteStream):
                    self.close()
    ...
    async def aiter_multipart(self) -> typing.AsyncIterator[MultipartPart]:
        ...
        with request_context(request=self._request):
            ...
            try:
                ...
                if is_streaming and isinstance(self.stream, AsyncByteStream):
                    await self.aclose()
