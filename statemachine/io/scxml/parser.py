...
def parse_state_data(state_elem: ET.Element) -> Dict[str, Any]:
    """Parse the ``<data>`` elements of a state's own ``<datamodel>`` as state data defaults.

    Only ``expr`` values that are Python literals are kept; other expressions are still
    evaluated by the document-wide datamodel.
    """
    data: Dict[str, Any] = {}
    for datamodel_elem in state_elem.findall("datamodel"):
        for data_elem in datamodel_elem.findall("data"):
            ...
            if expr is None:
                continue
            try:
                data[data_elem.attrib["id"]] = ast.literal_eval(expr.strip())
...
def parse_state(  # noqa: C901
    ...
    state = State(
        ...
        data=parse_state_data(state_elem),
    )
