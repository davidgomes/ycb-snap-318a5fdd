...
/**
 * `matchEach` builder. Every `.with()` / `.when()` clause is tested against
 * the input, and every matching handler result is collected.
 *
 * Patterns are checked against the original input type (`i`). `handledCases`
 * still narrows an internal remainder so `.exhaustive()` can verify coverage.
 * `.narrow()` applies that remainder to `i` for subsequent clauses.
 */
...
> = {
  ...
  /**
   * Calls `callback` once per result collected before this point.
   * Does not change the results array.
   */
  tap(
  ...
  ): [inferredOutput] extends [never]
    ? [PickReturnValue<o, c>]
    : PickReturnValue<o, inferredOutput>[] | [PickReturnValue<o, c>];
...
type ExhaustiveEach<output, inferredOutput> = {
  ...
  <otherOutput>(
    ...
    | PickReturnValue<output, inferredOutput>[]
    | [PickReturnValue<output, otherOutput>];
};
