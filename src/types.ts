...
export interface FetchOptions<R extends ResponseType = ResponseType, T = any>
   ...
   *
   * `true` uses the defaults (`threshold` 5, `cooldown` 30000,
   * `halfOpenMaxRequests` 1, and the standard failure statuses).
   * Omit it, or pass a falsey value, to disable circuit tracking.
   */
...
export interface CircuitBreakerOptions {
  ...
  /**
   * Milliseconds to stay open before a half-open probe is allowed.
   * @default 30000
   */
  cooldown?: number;

  /**
   * Concurrent half-open probes allowed for one origin.
   * @default 1
   */
