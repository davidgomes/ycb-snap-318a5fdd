...
function parseUsingDeclarationOrExpressionStatement(
  ...
  if ((parser.flags & Flags.NewLine) === 0 && isResourceBindingStart(parser.getToken())) {
    if (origin & Origin.TopLevel && context & Context.InGlobal && (context & Context.Module) === 0) {
      parser.report(Errors.UsingDeclarationInGlobal);
    }
  ...
  parser.assignable = AssignmentKind.Assignable;
  return finishUsingExpressionOrLabelledStatement(
    parser,
    context,
    scope,
    privateScope,
...
function finishUsingExpressionOrLabelledStatement(
  parser: Parser,
  context: Context,
  scope: Scope | undefined,
  privateScope: PrivateScope | undefined,
...
function parseForStatement(
  ...
  let isVarDecl =
    ...
    (parser.options.next && parser.getToken() === Token.UsingKeyword);
  ...
  } else if (parser.options.next && token === Token.AwaitKeyword && nextTokenIsUsingOnSameLine(parser)) {
    if (!(context & Context.InAwaitContext) && !(context & Context.Module && context & Context.InGlobal)) {
      parser.report(Errors.AwaitUsingOutsideAsync);
    }
    if (context & Context.InStaticBlock) parser.report(Errors.InvalidAwaitInStaticBlock);
