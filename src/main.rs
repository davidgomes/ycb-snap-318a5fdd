...
use crate::sort::{SortGroup, SortOptions};
...
fn construct_config(mut opts: Opts, pattern_regexps: &[String]) -> Result<Config> {
    ...
    let sort = extract_sort_options(&mut opts);
...
fn extract_sort_options(opts: &mut Opts) -> Option<SortOptions> {
    if opts.sort.is_empty() {
        return None;
    }

    let group = if opts.dirs_first {
        Some(SortGroup::DirsFirst)
    } else if opts.files_first {
        Some(SortGroup::FilesFirst)
    } else {
        None
    };
    ...
    Some(SortOptions {
        ...
        seed: opts.sort_seed.unwrap_or_else(sort::seed_from_time),
    })
}
