...
export function getAllCustomIgnoreSectionsInText(text: string): {startIndex: number, endIndex: number}[] {
  // Lazy require avoids a cycle: rule-ignore imports mdast positions.
  // eslint-disable-next-line no-undef
  const {getBlanketIgnoreSpans} = require('./rule-ignore') as typeof import('./rule-ignore');
  return getBlanketIgnoreSpans(text);
}
