...
export const applyDeleteSet = (deleteSet, transaction, store) => {
  ...
  deleteSet.clients.forEach(/** @param {IdRanges} idRanges @param {number} client */ (idRanges, client) => {
    const structs = store.clients.get(client) || []
    const state = getState(store, client)
    for (const deleteRange of idRanges.getIds()) {
      const clock = deleteRange.clock
      const clockEnd = clock + deleteRange.len
      if (clock < state) {
        if (state < clockEnd) {
...
export const readAndApplyDeleteSet = (decoder, transaction, store) =>
  applyDeleteSet(readIdSet(decoder), transaction, store)
