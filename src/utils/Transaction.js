...
import { abortMapTransaction, isMapConflictError } from './MapConflict.js'
...
export class Transaction {
  ...
  constructor (doc, origin, local) {
    ...
    /**
     * Map writes seen in this transaction.
     *
     * @type {Array<import('./MapConflict.js').MapWrite>}
     */
    this._mapWrites = []
    /**
     * Items already recorded as explicit map writes.
     *
     * @type {Set<Item>}
     */
    this._mapWriteItems = new Set()
    /**
     * @type {boolean}
     */
    this._aborted = false
...
export const transact = (doc, f, origin = null, local = true) => {
  ...
  try {
    ...
    if (isMapConflictError(err) && !doc._transaction._aborted) {
      doc._transaction._aborted = true
      abortMapTransaction(doc._transaction)
    }
    throw err
  } finally {
    if (initialCall) {
      ...
      if (finishCleanup) {
        ...
        if (doc._transactionCleanups[0]?._aborted) {
          doc._transactionCleanups = []
        } else {
