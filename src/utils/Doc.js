 ...
 * @property {MapConflictPolicy} [DocOpts.mapConflictPolicy='allow'] How conflicting map writes are handled.
 */
...
export class Doc extends ObservableV2 {
  ...
  /**
   * Return conflicts collected while applying local or remote updates.
   *
  ...
  /**
   * Return aggregate counts for collected map conflicts.
   *
  ...
  getMapConflictSummary () {
    /**
     * @type {{count:number,total:number,byType:{[key:string]:number},byKey:{[key:string]:number},byParent:{[key:string]:number},bySource:{[key:string]:number}}}
     */
    const summary = {
      count: this._mapConflicts.length,
      total: this._mapConflicts.length,
      byType: {},
      byKey: {},
      byParent: {},
      bySource: {}
    }
    for (const conflict of this._mapConflicts) {
