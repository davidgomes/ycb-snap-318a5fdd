 ...
 * @property {import('./MapConflicts.js').MapConflictPolicy} [DocOpts.mapConflictPolicy='allow'] How conflicting writes to the
 * same map key within a transaction or merged update are handled. 'allow' applies them normally, 'collect' applies them and
 * records the conflicts (see `getMapConflicts()`), 'error' throws a `MapConflictError` before the conflicting write applies.
 */
...
export class Doc extends ObservableV2 {
  ...
  /**
   * Map conflicts recorded with `mapConflictPolicy: 'collect'`, in the order they were detected.
   *
