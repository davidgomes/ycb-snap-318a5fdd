  ...
  implements OrderByInterface<DB, TB, {}>, AliasableExpression<O>
{
   ...
   *
   * This is meant for value window functions such as {@link FunctionModule.lag},
   * {@link FunctionModule.lead}, {@link FunctionModule.firstValue},
   * {@link FunctionModule.lastValue} and {@link FunctionModule.nthValue}, and
   * is only supported by some dialects like MS SQL Server.
   *
   * ### Examples
   *
   * ```ts
   ...
   *
   * ```sql
   * select lag("first_name") respect nulls over(order by "id") as "previous_first_name"
   * from "person"
   * ```
   */
   ...
   *
   * See {@link respectNulls} for details.
   *
   * ### Examples
   *
   * ```ts
