...
/**
 * Removes window-frame clauses that only restate the SQL-standard default.
 *
 * When an `over` clause has `order by`, the database already applies
 * `range between unbounded preceding and current row`. When it has no
 * `order by`, the default is
 * `range between unbounded preceding and unbounded following`.
 *
 * Frames that use `rows` or `groups`, an exclusion clause, a non-default
