...
fn char_ranges(expr: &OptimizedExpr) -> Option<Vec<(char, char)>> {
    match expr {
        ...
        OptimizedExpr::Range(start, end) => range_pair(start, end).map(|range| vec![range]),
        OptimizedExpr::CharClass(ranges) => ranges
            .iter()
            .map(|(start, end)| range_pair(start, end))
            .collect(),
...
fn range_pair(start: &str, end: &str) -> Option<(char, char)> {
    ...
    for (start, end) in ranges {
        if let Some((_, previous_end)) = merged.last_mut() {
            let adjacent = (*previous_end as u32)
                .checked_add(1)
                .is_some_and(|next| start as u32 <= next);
...
mod tests {
    ...
    fn simplifies_a_single_merged_range() {
        let adjacent = OptimizedExpr::Choice(
            Box::new(OptimizedExpr::Str("a".to_owned())),
            Box::new(OptimizedExpr::Str("b".to_owned())),
        );
        assert_eq!(
            coalesce(rule(adjacent)).expr,
            OptimizedExpr::Range("a".to_owned(), "b".to_owned())
        );

        let same = OptimizedExpr::Choice(
            Box::new(OptimizedExpr::Str("a".to_owned())),
            Box::new(OptimizedExpr::Range("a".to_owned(), "a".to_owned())),
        );
        assert_eq!(
            coalesce(rule(same)).expr,
    ...
    #[test]
    fn coalesces_contiguous_qualifying_runs() {
        let expr = OptimizedExpr::Choice(
            Box::new(OptimizedExpr::Str("a".to_owned())),
            Box::new(OptimizedExpr::Choice(
                Box::new(OptimizedExpr::Str("b".to_owned())),
    ...
    #[test]
    fn absorbs_existing_char_class_ranges() {
        let expr = OptimizedExpr::Choice(
            ...
            Box::new(OptimizedExpr::Choice(
                Box::new(OptimizedExpr::Str("d".to_owned())),
                Box::new(OptimizedExpr::Str("w".to_owned())),
            )),
        );
        ...
        assert_eq!(
            ...
            OptimizedExpr::CharClass(vec![
                ...
                ("w".to_owned(), "z".to_owned()),
            ])
        );
    }

    #[test]
    fn skips_coalesce_when_ranges_are_not_fewer() {
        let expr = OptimizedExpr::Choice(
            Box::new(OptimizedExpr::Str("a".to_owned())),
            Box::new(OptimizedExpr::Str("c".to_owned())),
        );
