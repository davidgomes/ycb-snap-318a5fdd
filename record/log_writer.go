...
func (q *syncQueue) pop(head, tail uint32, err error, queueSemChan chan struct{}) error {
	...
	for ; tail != head; tail++ {
		...
		*slot.err = err
		...
		// queue.
		q.headTail.Add(1)
		// Invoke OnSync before Done so waiters that wake observe the callback's
		// stores.
		if onSync != nil {
			onSync(err)
		}
...
// SyncRecordWithCallback is SyncRecord with an optional callback invoked when
// the sync completes, before the wait group is signaled. onSync must not block.
