...
Register-ArgumentCompleter -CommandName task -ScriptBlock {
	...
	if ($commandName.StartsWith('-')) {
		$completions = @(
			...
			[CompletionResult]::new('--no-status', '--no-status', [CompletionResultType]::ParameterName, 'ignore status in JSON or graph output'),
			[CompletionResult]::new('--graph', '--graph', [CompletionResultType]::ParameterName, 'print the task dependency graph'),
			[CompletionResult]::new('--format', '--format', [CompletionResultType]::ParameterName, 'graph output format'),
			[CompletionResult]::new('--reverse', '--reverse', [CompletionResultType]::ParameterName, 'show tasks that depend on the given task'),
