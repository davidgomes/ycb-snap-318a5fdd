...
JSON responses can be streamed value by value. ``application/json`` arrays yield each element, while ``application/ndjson``, ``application/x-ndjson``, and ``application/json-seq`` yield one value per record.
