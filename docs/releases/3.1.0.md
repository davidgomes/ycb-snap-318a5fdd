...
States can now own data scoped to their lifecycle. Declare it with the `data`
keyword (also accepted as a class keyword by `State.Compound` and `State.Parallel`):
it is initialized on entry, removed on exit, reset on re-entry, and stored per
instance. Values can be plain defaults, factories, or `DataVar` declarations with
optional type enforcement.

```python
class Checkout(StateChart):
    cart = State(initial=True, data={"items": list, "total": DataVar(0, type=int)})
    ...

    def on_exit_cart(self, state_data):
        print(state_data["items"])
```

Callbacks receive a `state_data` mapping merging the data of the state and its
active ancestors. History states restore data snapshots, and new runtime methods
`get_state_data()`, `state_data_values`, `set_state_data()` and `get_data_changes()`
inspect and update it. SCXML state `<datamodel>` literals and diagrams are supported.
See {ref}`state-data` for details.
