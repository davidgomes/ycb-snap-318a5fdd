...
States can now own data with the new `data` keyword. The data is initialized from the
declared defaults on every entry, discarded on exit, stored per machine instance and
restored by history pseudo-states. Callbacks receive it through the new `state_data`
parameter, merged with the data of the ancestor states:
...
...     downloading = State(data={"retries": DataVar(0, type=int), "chunks": list})
...
...
See {ref}`state-data` for scoping rules, `DataVar` type enforcement and factories,
`set_state_data()`, `state_data_values` and `get_data_changes()`.
