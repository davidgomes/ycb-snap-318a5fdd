...
:func:`~.result_to_validated` turns ``Success`` into ``Valid`` and wraps a
``Failure`` error in a one-element tuple.
:func:`~.validated_to_result` turns ``Valid`` into ``Success`` and ``Invalid``
into ``Failure`` holding that error tuple.
