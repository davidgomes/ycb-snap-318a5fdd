...
- :func:`~.result_to_validated` converts ``Result`` to ``Validated``
- :func:`~.validated_to_result` converts ``Validated`` to ``Result``

``Failure`` errors are wrapped in a 1-tuple.
``Invalid`` keeps that tuple when converting back to ``Failure``.
