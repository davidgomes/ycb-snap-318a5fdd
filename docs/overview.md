...
|:white_check_mark:[brokenDocLink](#brokendoclink)|Detects doc comment symbol links that do not resolve|
...
Detects doc comment symbol links that do not resolve.

Parses bracket links such as [Name], [pkg.Name], and [pkg.Type.Method] with go/doc/comment and checks them against the current package and the file's imports.



**Before:**
```go
// F delegates to [Missing].
func F() {}
```

**After:**
```go
// F delegates to [Helper].
func F() {}
```
