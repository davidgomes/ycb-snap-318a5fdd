...
### Config-level options:
    ...
    report_file:                 [String]  file to write test results to (stdout). Supports `<launcher>`, `<date>` (YYYY-MM-DD) and `<timestamp>` (YYYY-MM-DD_HH-MM-SS). When `<launcher>` is present, each browser writes its own file.
    ...
    tap_show_launcher_summary    [Boolean] append a per-launcher pass/fail/skip summary to TAP output
    ...
    xunit_include_launcher_properties [Boolean] include launcher pass/fail properties in xunit XML
