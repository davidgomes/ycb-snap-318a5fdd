...
### Config-level options:
    ...
    report_file:                 [String]  file to write test results to (stdout). Supports the template variables `<launcher>` (one file per launcher, e.g. `reports/<launcher>.xml`), `<date>` (YYYY-MM-DD) and `<timestamp>` (YYYY-MM-DD_HH-MM-SS)
    ...
    tap_show_launcher_summary    [Boolean] append per-launcher pass/fail/skip counts to the TAP summary (false)
    ...
    xunit_include_launcher_properties [Boolean] include launcher metadata as `<properties>` in xunit output (false)
