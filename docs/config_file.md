...
### Config-level options:
    ...
    report_file:                 [String]  file to write test results to (stdout); supports `<launcher>`, `<date>` and `<timestamp>` templates (see below)
    ...
    tap_show_launcher_summary    [Boolean] append a per-launcher pass/fail/skip summary to TAP output (false)
    ...
    xunit_include_launcher_properties [Boolean] add a `<properties>` block with launcher metadata to xunit output (false)
    xunit_intermediate_output    [Boolean] print tap output for the xunit reporter (false)

### Report file templates:

`report_file` may contain the following template variables:

    <launcher>                   the launcher name, made filesystem-safe (`/\:*?"<>|()` each become `_`, whitespace runs become a single `_`)
    <date>                       the date the run started, as YYYY-MM-DD
    <timestamp>                  the time the run started, as YYYY-MM-DD_HH-MM-SS

When `<launcher>` is present, each launcher's results are written to a separate file (e.g. `results/<launcher>.xml` produces `results/Headless_Chrome.xml`, `results/Firefox.xml`, ...) while stdout still receives the combined results. Results from Testem itself (e.g. failing hooks) are only written to stdout. Unknown template variables are reported as a configuration error, and using `<launcher>` without a file extension logs a warning.
