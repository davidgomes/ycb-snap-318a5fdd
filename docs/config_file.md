...
### Config-level options:
    ...
    bail_on_test_failure:        [Boolean|Number] stop the run and abort all launchers once this many tests have failed (skipped and todo tests do not count); `true` bails on the first failure, a positive integer N on the Nth (false)
