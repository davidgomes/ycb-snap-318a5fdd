...
### Config-level options:
    ...
    report_file:                 [String]  file to write test results to. Supports `<launcher>`, `<date>` (YYYY-MM-DD), and `<timestamp>` (YYYY-MM-DD_HH-MM-SS). When `<launcher>` is present, each browser is written to its own file; stdout still receives the combined results. The internal `testem` launcher is not written to a file. Launcher names are made filesystem-safe.
    ...
    tap_show_launcher_summary    [Boolean] append a "Per-launcher summary" (`N tests, N pass, N fail, N skip`) to TAP output
    ...
    xunit_include_launcher_properties [Boolean] include launcher, launchers, and `<launcher>_pass` / `<launcher>_fail` properties in xunit XML
