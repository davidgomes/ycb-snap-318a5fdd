...
### CLI-level options:
    ...
    bail_on_test_failure:    [Boolean|Number] stop the run after a test failure. `true` bails on the first failure; a positive integer N bails on the Nth failure (false)
