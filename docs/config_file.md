...
### CLI-level options:
    ...
    bail_on_test_failure:    [Boolean|Number] stop after N non-skipped, non-todo failures. `true` means 1, a positive integer means that threshold, `false` (default) disables. Invalid values warn and act as `false`
