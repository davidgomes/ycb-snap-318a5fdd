...
* `def .iter_multipart()` - **multipart part iterator**
* `def .close()` - **None**
* `def .next()` - **Response**
* `def .aread()` - **bytes**
...
* `def .aiter_multipart()` - **async multipart part iterator**
* `def .aclose()` - **None**
* `def .anext()` - **Response**

## `MultipartPart`

*A single part from a `multipart/*` response, as yielded by `Response.iter_multipart()` and `Response.aiter_multipart()`.*
