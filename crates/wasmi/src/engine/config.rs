...
pub struct Config {
    ...
    /// Is `true` if Wasmi shall generate coredumps upon Wasm traps.
...
impl Config {
    ...
    /// # Note
    ///
    /// When enabled, errors caused by Wasm traps carry a coredump in the
    /// [Wasm coredump format] which can be queried via [`Error::coredump`].
    ///
    /// Disabled by default.
    ///
