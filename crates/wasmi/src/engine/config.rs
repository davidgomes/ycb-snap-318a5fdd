...
pub struct Config {
    ...
    /// Is `true` if Wasmi shall generate a coredump when a Wasm trap occurs.
    generate_coredump: bool,
    /// The name of the executable recorded in generated coredumps.
...
impl Config {
    ...
    /// Configures whether Wasmi generates a coredump when a Wasm trap occurs.
    ///
    /// # Note
    ///
    /// - The generated coredump is accessible via [`Error::coredump`] of the returned [`Error`].
    /// - Coredumps are only generated for Wasm traps, not for errors returned by host functions.
    /// - Coredumps follow the [Wasm coredump] conventions and can be inspected by post-mortem
    ///   debugging tools.
    /// - Enabling coredumps records additional information about compiled functions.
    ///
    /// Disabled by default.
    ///
    /// [`Error`]: crate::Error
    ...
    /// Returns `true` if the [`Config`] enables coredump generation upon Wasm traps.
    pub(crate) fn get_generate_coredump(&self) -> bool {
        self.generate_coredump
    }

    /// Sets the name of the executable recorded in coredumps generated by the [`Engine`].
    ///
    /// Default value: `""`
    ///
    /// [`Engine`]: crate::Engine
    ...
    /// Returns the name of the executable recorded in generated coredumps.
